...
class HappyClient():
    ...
    async def connect(self, dev_or_addr: BLEDevice | str):
        self.client = BleakClient(
            dev_or_addr, disconnected_callback=self.handle_disconnect)
        try:
    ...
    async def disconnect(self):
        try:
            # run in background because it hangs
    ...
    async def set_mode(self, mode: Mode | int, speed=0):
    ...
    def get_modes(self):
        modes = [{'name': mode.name, 'value': mode.value} for mode in Mode]
        return modes
