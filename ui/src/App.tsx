...
function App() {
  ...
  useEffect(() => {
    const debouncedColorChange = _.debounce(async (newColor) => {
  ...
  async function scan() {
    ...
    if (res.data?.length > 0) {
      setSelectedDev(res.data?.[0]?.address);
    }
    setScanning(false);
  }
  ...
  return (
    <div className="w-full h-full">
      <div className="flex flex-col w-[300px] gap-3 m-auto">
        {!connected && (
        ...
        {connected && (
          <>
            <button
              className="btn btn-circle btn-outline self-center mt-5  "
